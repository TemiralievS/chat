...
class ClientTransport(threading.Thread, QObject):
    ...
    def client_message_presence(self):
        out = {
        ...
        client_logger.debug(f'Сформировано {jim_presence} сообщение для пользователя {self.username}')
        return out
    ...
    def server_message(self, message):
        ...
        if jim_response in message:
            if message[jim_response] == 200:
                ...
                client_logger.debug(f'Принят неизвестный код подтверждения {message[jim_response]}')
            ...
            client_logger.debug(f'Получено сообщение от пользователя {message[jim_sender]}:{message[jim_message_txt]}')
